...
window.ProductView = (function(superClass) {
  ...
  ProductView.prototype.addToCart = function(e) {
    ...
    if (Modernizr.cssanimations) {
      ...
      this.$productMessage.addClass("hidden");
    } else {
    ...
    if (quantity === "" || quantity === "0") {
      return setTimeout((function(_this) {
        return function() {
          ...
          _this.$productMessage.removeClass("hidden");
        ...
        return function(cartItem) {
          return setTimeout(function() {
            ...
            Shopify.getCart(function(cart) {
              return $(".header-cart-count .count").text(cart.item_count);
            });
...
window.ImagesWithVideoView = (function(superClass) {
  extend(ImagesWithVideoView, superClass);

  function ImagesWithVideoView() {
    return ImagesWithVideoView.__super__.constructor.apply(this, arguments);
  }

  ImagesWithVideoView.prototype.events = {
    'click [data-overlay-play]': 'activateVideo',
    'click .product-details-slideshow-nav-list-item': 'specificSlide'
  };

  ImagesWithVideoView.prototype.initialize = function() {
  ...
  ImagesWithVideoView.prototype.activateVideo = function(event) {
  ...
  ImagesWithVideoView.prototype.specificSlide = function(event) {
    var index;
  ...
  ImagesWithVideoView.prototype.updateSlidePagination = function(index) {
  ...
  ImagesWithVideoView.prototype.showNewSlide = function(index) {
...
// Previous and next buttons for the featured products on front page.
$(document).ready(function() {
  // Previous
  $('.product-details-slideshow-prev-item').on('click', function() {
    var $currentSlide = $('.product-details-slideshow-list-item.active');
    var $currentNav = $('.product-details-slideshow-nav-list-item.active');
    var $prevSlide, $prevNav;
    $currentSlide.removeClass('active');
    $currentNav.removeClass('active');
    if ($currentSlide.prev('.product-details-slideshow-list-item').length > 0) {
      $prevSlide = $currentSlide.prev('.product-details-slideshow-list-item');
      $prevNav = $currentNav.parent().prev().children();
    } else {
      $prevSlide = $('.product-details-slideshow-list-item').last();
      $prevNav = $('.product-details-slideshow-nav-list li:last-child a');
    }
  ...
  // Next
  $('.product-details-slideshow-next-item').on('click', function() {
    var $currentSlide = $('.product-details-slideshow-list-item.active');
    var $currentNav = $('.product-details-slideshow-nav-list-item.active');
    var $nextSlide, $nextNav;
    $currentSlide.removeClass('active');
    $currentNav.removeClass('active');
    if ($currentSlide.next('.product-details-slideshow-list-item').length > 0) {
      $nextSlide = $currentSlide.next('.product-details-slideshow-list-item');
      $nextNav = $currentNav.parent().next().children();
    } else {
      $nextSlide = $('.product-details-slideshow-list-item').first();
      $nextNav = $('.product-details-slideshow-nav-list li:first-child a');
    }
...
window.HomeView = (function(superClass) {
  ...
  HomeView.prototype.sectionBinding = function() {
    ...
    this.sections.register('images-with-video', this.imagesWithVideo(this.sections));
  ...
  HomeView.prototype.imagesWithVideo = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        return this.instances[instance.sectionId] = new ImagesWithVideoView({
          el: instance.$container
        });
      },
      onSectionLoad: function(event) {
...
window.PageView = (function(superClass) {
  ...
  PageView.prototype.sectionBinding = function() {
    this.sections = new ThemeEditor();
  ...
  PageView.prototype.fullWidthFeature = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        var $video;
...
// Show price formatted similar to bold for products that don't have a subscription option
$(window).load(function() {
  if ($('.recurring_div').length == 0) {
    var currentPrice = $('.productPrice').text();
    $('.one_time_radio_lbl span').html(
      "<span class='boldCurrentPrice'>" + currentPrice + "</span>" +
      "<span class='boldCurrentText'>one time purchase</span>"
    );
    var discountPrice = $('.new_discounted_price').text();
    var $recurringWrapper = $('.recurring_radio_lbl');
    // Get only text because we want the price up front
    var discountText = $recurringWrapper.text().split('—');
    discountText = discountText[0].toString().substring(1);
    $('.recurring_radio_lbl input').nextAll().remove();
    // Remove text that is not in an element
    $recurringWrapper.contents().filter(function() {
      return this.nodeType === 3;
    }).remove();
    // Add properly formatted text back in
    $recurringWrapper.append(
      "<span><span class='boldDiscountPrice'>" + discountPrice + "</span>" +
      "<span class='boldDiscountText'>" + discountText + "</span></span>"
    );
    $('.product_rp_div.noSub').fadeIn(300);
  } 
});
